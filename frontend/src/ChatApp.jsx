...
export default function App () {
  const [apiKey, setApiKey] = useState(() =>
    (localStorage.getItem('or_key') || '').trim()
  )
  ...
  const saveKey = k => {
    ...
    localStorage.setItem('or_key', trimmedKey)
  }

  const sendMessage = async text => {
    const trimmedKey = apiKey.trim()
    if (!text.trim() || !trimmedKey) {
      setMessages(prev => [
        ...prev,
        {
          role: 'assistant',
          content: '⚠️ الرجاء إدخال مفتاح OpenRouter API صحيح قبل المتابعة.'
        }
      ])
      return
    }
    ...
    try {
      ...
      let errorMsg = `⚠️ حدث خطأ: ${e.message}`
      if (/api key|apiKey|مفتاح/i.test(e.message)) {
        errorMsg += '\nتحقق من صحة مفتاح OpenRouter API وحاول مرة أخرى.'
      } else if (/network|failed|fetch/i.test(e.message)) {
  ...
  const handleStart = async () => {
    ...
    setProjectData(null)
    await sendMessage(
      'ابدأ الحوار معي لتقدير مشروعي البرمجي. قدم نفسك واطرح أول سؤال.'
    )
  }
            ...
            <div className='sidebar-section'>
              ...
              <div className='metric-card'>
                ...
                <div className='metric-value'>
                  {projectData.fpResults.fp.toFixed(1)}
                </div>
              ...
              <div className='metric-card'>
                ...
                <div className='metric-value'>
                  {projectData.ucpResults.ucp.toFixed(1)}
                </div>
                <div className='progress-bar-wrap'>
                  <div className='progress-bar-bg'>
                    <div
              ...
              <div className='metric-card'>
                ...
                <div className='metric-value'>
                  {Math.round(
                    (projectData.fpResults.effortHours +
                      projectData.ucpResults.effortHours) /
                      2
                  ).toLocaleString()}
                </div>
              ...
              <div className='metric-card'>
                ...
                <div className='metric-value' style={{ fontSize: '16px' }}>
                  {formatCurrency(
                    (projectData.fpResults.cost + projectData.ucpResults.cost) /
                      2
                  )}
                </div>
            ...
            <>
              <div className='messages'>
                {messages.map((m, i) => (
                  <div
                    key={i}
                    className={`msg-wrap ${m.role === 'user' ? 'user' : ''}`}
                  >
                    <div
              ...
              {projectData && (
                <div className='results-panel'>
                  <div className='results-title'>
                    ✅ اكتمل التحليل — نتائج التقدير
                  </div>
                  <div className='results-grid'>
                    <div className='result-item'>
                      <div className='result-method'>FP — Function Points</div>
                      <div className='result-val fp'>
                        {projectData.fpResults.fp.toFixed(1)}
                      </div>
                      <div className='result-sub'>
                        {formatHours(projectData.fpResults.effortHours)} ·{' '}
                        {formatCurrency(projectData.fpResults.cost)}
                      </div>
                    </div>
                    <div className='result-item'>
                      <div className='result-method'>UCP — Use Case Points</div>
                      <div className='result-val ucp'>
                        {projectData.ucpResults.ucp.toFixed(1)}
                      </div>
                      <div className='result-sub'>
                        {formatHours(projectData.ucpResults.effortHours)} ·{' '}
                        {formatCurrency(projectData.ucpResults.cost)}
                      </div>
                    </div>
                    <div className='result-item' style={{ gridColumn: '1/-1' }}>
                      ...
                      >
                        {formatCurrency(
                          (projectData.fpResults.cost +
                            projectData.ucpResults.cost) /
                            2
                        )}
                      </div>
                      <div className='result-sub'>
